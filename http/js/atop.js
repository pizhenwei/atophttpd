...
function main() {
    ...
    if (delta == 0 && init == true) {
        init = false
        getHtmlTemplateFromServer(req_timestamp, req_template_type, req_change_data, req_change_template).then(getJsonRawDataFromServer).then(res => {
            LoadHtmlFromJson(cache_json_data, cache_template)
            while (p = queue.shift()) {
                p.resolve()
            }
            loading = false
...
function getHtmlTemplateFromServer(req_timestamp, req_template_type, req_change_data, req_change_template) {
    return new Promise((resolve, reject) => {
        console.log("req_timestamp: " + req_timestamp)
            ...
            if (req_change_template) {
                ...
                request.onload = function () {
                    if (request.status === 200) {
                        raw_template = request.responseText;
                        cache_template = raw_template
                        resolve({
                            req_timestamp,
                            req_change_data
                        })
...
function parseAtopHeader(json) {
    ...
    //CPU line
    CPUEntry = json[0]["CPU"];
    CPU_Tot = CPUEntry["stime"] + CPUEntry["utime"] + CPUEntry["ntime"] +
